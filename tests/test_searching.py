...
Covers response structure, result ordering, limit, ef_search, per-field query
format, meta round-trips, and edge cases (oversized limit, empty fields dict).
"""

import json

import pytest
from helpers import DENSE_FIELD, N_VECTORS, dense_vec
...
def test_search_result_has_required_keys(populated_collection):
    ...
    results = collection.search(
        fields={DENSE_FIELD: {"query": dense_vec(), "limit": 1}}
    )["results"][DENSE_FIELD]
...
def test_search_results_ordered_by_descending_similarity(populated_collection):
    ...
    _, collection = populated_collection
    results = collection.search(
        fields={DENSE_FIELD: {"query": dense_vec(), "limit": 10}}
    )["results"][DENSE_FIELD]
...
def test_search_limit_returns_at_most_n_results(populated_collection, limit):
    ...
    _, collection = populated_collection
    results = collection.search(
        fields={DENSE_FIELD: {"query": dense_vec(), "limit": limit}}
    )["results"][DENSE_FIELD]
...
def test_search_meta_content_round_trips(empty_collection):
    ...
    results = collection.search(
        fields={DENSE_FIELD: {"query": dense_vec(seed=77), "limit": 1}}
    )["results"][DENSE_FIELD]
