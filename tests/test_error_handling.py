...
def test_create_index_dim_1_raises(client):
    ...
    with pytest.raises(ValueError):
        client.create_index(
            name=uid("d1"), dimension=1, space_type="cosine", precision=Precision.INT8
        )
...
def test_create_index_dim_2_is_valid(client):
    ...
    try:
        client.create_index(
            name=name, dimension=2, space_type="cosine", precision=Precision.INT8
        )
    finally:
        safe_delete(client, name)
...
def test_query_prefilter_over_1000000_raises(populated_index):
    ...
    with pytest.raises(ValueError):
        index.query(
            vector=dense_vec(), top_k=5, prefilter_cardinality_threshold=1_000_001
        )
